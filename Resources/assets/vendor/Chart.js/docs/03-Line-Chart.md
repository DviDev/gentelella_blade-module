...
The line chart usually requires an array of labels. This labels are shown on the X axis. There must be one label for
...
We can also change these defaults values for each Line type that is created, this object is available at
`Chart.defaults.line`.
